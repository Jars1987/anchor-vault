...
fn test_withdraw() {
    ...
    let transfer_amount = 500_000_000;
    let data = (vault::instruction::Withdraw { amount: transfer_amount }).data();
    ...
    let checks = &vec![Check::success(), Check::account(vault_bind).lamports(0).build()];
...
fn test_close() {
    ...
    let vault_bind = &vault_pda;
    let checks = &vec![Check::success(), Check::account(vault_bind).closed().build()];
